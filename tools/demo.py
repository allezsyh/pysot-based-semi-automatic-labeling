...
parser.add_argument('--video_name', default='Y:/children_hospital_dataset/select_frames/1/camera1-2021_12_14_08-00-01', type=str, help='videos or image files')
parser.add_argument('--txt_root', default='Y:/children_hospital_dataset/other_labels/3', help='datasets')
parser.add_argument('--cls_id', default=3, help='class id')
...
def get_frames(video_name):
    ...
    if os.path.exists('last.txt'):
        with open('last.txt', "r") as rl:
            last = rl.readline()
        resume_split = int(last.split(os.sep)[-2].split('_')[-1])
        resume_txt_id = int(last.split('_')[-1].split('.')[0])
    for split_id in range(1, len(os.listdir(video_name)) + 1):
        split = 'split_' + str(split_id)
        if split_id < resume_split:
            continue
        img_path = os.path.join(video_name, split)
        images_split = glob(os.path.join(img_path, '*.jp*'))
...
def main():
    ...
    first_frame = True
    for frame, frame_path in get_frames(args.video_name):
...
def human_plot():
    ...
    for i, im in enumerate(ims):
        ...
        cv2.moveWindow('human plot', 300, 100)
