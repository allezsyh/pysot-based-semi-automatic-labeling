...
parser.add_argument('--start_video', default='camera2-2021_12_26_06-00-01_1', type=str, help='videos or image files')
...
parser.add_argument('--cls_id', default=30, help='class id')
args = parser.parse_args()
basetxt = '3_room_2_c2.txt'
global_flag = True
    ...
    for one_split_id in split_i_ls:
        ...
        for frame, frame_path in get_frames(one_split_path):
            ...
            if first_frame:
                ...
                with open(basetxt, "r") as rl:
                    bt = rl.readline().strip("\n").split()
                if not bt:
                    if os.path.exists((str(args.cls_id)+'_last_split.txt')):
                ...
                with open(basetxt, "r") as rl:
                    bt = rl.readline().strip("\n").split()
                target_pos[0] = img_w * float(bt[1])
                target_pos[1] = img_h * float(bt[2])
                target_sz = [w, h]
                target_sz[0] = img_w * float(bt[3])
                target_sz[1] = img_h * float(bt[4])
...
def human_plot():
    ...
    for i, im in enumerate(ims):
        ...
        with open(basetxt, "w") as out_file:
            log_id = cls_id
            if 10 <= cls_id < 100:
                log_id = cls_id // 10
            if 0 < args.cls_id < 1:  log_id = int(args.cls_id)
